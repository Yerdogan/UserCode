...
process.g4SimHits.MTTSD=cms.PSet(EnergyThresholdForHistoryInGeV = cms.double(0.005),EnergyThresholdForPersistencyInGeV = cms.double(0.01))
...
# definition of output file (full path)
    fileName = cms.untracked.string('FullSim_withMTT_Nev'+str(nevents)+'_'+guntypename+'_Pt'+str(plow)+'-'+str(phigh)+'GeV_Eta'+str(elow)+'-'+str(ehigh)+'_PU'+str(npu)+'.root'),
...
process.p2 = cms.Path(process.randomEngineStateProducer*process.mix*process.simSiPixelDigis)
